...
public class InitialDataDb implements IInitDB {
    ...
    public void InitializeItems() {
        if(dbService.getETaskService().getTasks().size()==0)
        {
            ...
            //словари
            String idForAllDictionary = dbService.getEDictionaryService().createDictionary("Для всех","cost_new64",null,"Самые простые слова для всех возврастов",uuidRus,iddifficultyVeryEasy);
            String idDifficultDictionary = dbService.getEDictionaryService().createDictionary("Сложный набор","cost_dollar64","0,5","Сложный набор слов, специфическая терминология",uuidRus,iddifficultyHard);
            String idBasicDictionary = dbService.getEDictionaryService().createDictionary("Базовый","cost_new64","2,99","Самые простые слова для всех возврастов",uuidRus,iddifficultyEasy);
            String idFilmsDictionary = dbService.getEDictionaryService().createDictionary("Фильмы","cost_free64",null,"Сложный набор слов, специфическая терминология",uuidRus,iddifficultyVeryHard);
